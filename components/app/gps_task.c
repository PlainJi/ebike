...
{
    if (esp_gps->item_str[0] != '\0') {
        esp_gps->parent.tim.hour = (convert_two_digit2number(esp_gps->item_str + 0) + TIME_ZONE) % 24;
...
static void parse_rmc(esp_gps_t *esp_gps)
{
    ...
    case 6: /* Longitude east(1)/west(-1) information */
        ...
        break;
    case 7: /* Process ground speed in unit km/h */
    ...
    case 9: /* Process date */
        if (esp_gps->item_str[0] != '\0') {
            ...
            esp_gps->parent.date.year = convert_two_digit2number(esp_gps->item_str + 4) + YEAR_BASE;
        }
        break;
...
static void parse_vtg(esp_gps_t *esp_gps)
{
    ...
    case 3:/* Process magnetic variation */
        esp_gps->parent.variation = strtof(esp_gps->item_str, NULL);
        break;
    case 5:/* Process ground speed in unit km/h */
        esp_gps->parent.speed = strtof(esp_gps->item_str, NULL) * 1.852;    //knots to km/h
        break;
    case 7:/* Process ground speed in unit km/h */
        esp_gps->parent.speed = strtof(esp_gps->item_str, NULL);            //km/h to m/s
        break;
    default:
        break;
    }
            ...
            if (esp_gps->crc == crc) {
                ...
                case STATEMENT_GSV:
                    if (esp_gps->sat_num == esp_gps->sat_count) {
                        if (strstr(esp_gps->buffer, "GPGSV")) {
                            esp_gps->parent.sats_in_view_total += esp_gps->parent.sats_in_view;
                            esp_gps->gpgsv_done = 1;
                        }
                        else if (strstr(esp_gps->buffer, "BDGSV")) {
                            esp_gps->parent.sats_in_view_total += esp_gps->parent.sats_in_view;
                            esp_gps->bdgsv_done = 1;
                        }
                    }
                    if(esp_gps->gpgsv_done && esp_gps->bdgsv_done) {
                ...
                if (((esp_gps->parsed_statement) & esp_gps->all_statements) == esp_gps->all_statements) {
                    ...
                    ESP_ERROR_CHECK(esp_event_post_to(event_handler, TASK_EVENTS, EVENT_UPDATE_GPS, &(esp_gps->parent), sizeof(gps_t), 100 / portTICK_PERIOD_MS));
                    esp_gps->buffer[len] = '\0';
                    ESP_ERROR_CHECK(esp_event_post_to(event_handler, TASK_EVENTS, EVENT_UPDATE_LOG, esp_gps->buffer, len, 100 / portTICK_PERIOD_MS));
                }
            } else {
...
nmea_parser_handle_t nmea_parser_init(const nmea_parser_config_t *config)
{
    ...
    memset(esp_gps, 0, sizeof(esp_gps_t));
